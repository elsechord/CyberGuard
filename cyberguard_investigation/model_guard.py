...
class ModelGuard:
    def __init__(self, config, directory, *, transport=None):
        ...
        # Provider keys have provider-defined formats. Strength requirements apply
        # to the credentials generated by CyberGuard, not to an external API key.
        upstream_key = config["upstream_key"]
        if not isinstance(upstream_key, str) or not upstream_key.strip() or any(c in upstream_key for c in ('\r', '\n')):
            raise ValueError("A nonempty provider API key is required")
        if any(not isinstance(value, str) or len(value) < 32 for value in [config["admin_token"], *credentials.values()]) or len(set(self.secrets)) != len(self.secrets):
